...
# avoiding rewrite, in case of multiple Memristor
    ...
    text +='if(Omegaold_CM < 0.001)\n'
    text +='{Omegaold_CM = 0.002;\n'
    text +='}\n'

    text +='if(Omegaold_CM > 0.99)\n'
    text +='{Omegaold_CM = 0.98;\n'
    text +='}\n'

    text +='if(Omegaxold_CM < 0.001)\n'
    text +='{Omegaxold_CM = 0.002;\n'
    text +='}\n'

    text +='if(Omegaxold_CM > 0.99)\n'
    text +='{Omegaxold_CM = 0.98;\n'
    text +='}\n'
    ...
    text +='\tFwv =(2*(Omegaold_CM-Omegaxold_CM)/'+comp['hh']+')-(1.e0*((((Un_CM*Un_CM)/Rch_W)-(Gth_W*'+comp['tem']+'))/dH_W));\n'
    text +='\tJw = (2/'+comp['hh']+')+1.e0*(((((Un_CM*Un_CM)/Rch_W)-(Gth_W*'+comp['tem']+'))*((2.*'+comp['dh']+')+ddH_I+ddH_II+ddH_III))/(dH_I*((dH_II + dH_III)*(dH_II + dH_III))))-(1.e0*((dRch+dGth)/dH_W));\n'
