...
# avoiding rewrite, in case of multiple Memristor
    ...
    text +='if(Omegaold_CM < 0.000001)\n'
    text +='{Omegaold_CM = 0.001;\n'
    text +='}\n'

    text +='if(Omegaold_CM > 0.99999)\n'
    text +='{Omegaold_CM = 0.99;\n\n'
    text +='}\n'

    text +='if(Omegaxold_CM < 0.000001)\n'
    text +='{Omegaxold_CM = 0.001;\n'
    text +='}\n'

    text +='if(Omegaxold_CM > 0.99999)\n'
    text +='{Omegaxold_CM = 0.99;\n\n'
    text +='}\n'
    ...
    text +='\tRch_I = (('+comp['ins']+' * 2)/(M_PI * (3* '+comp['rch']+')));\n'
    text +='\tRch_II = 1.+((('+comp['ins']+' / '+comp['met']+')-1.) * (Omegaold_CM * Omegaold_CM));\n'
    text +='\tRch_W = Rch_I / Rch_II;\n'
    text +='\tdRch = 2. * ((('+comp['ins']+'/'+comp['met']+')-1.)*M_PI*('+comp['rch']+'*3)*(Un_CM * Un_CM)*Omegaold_CM)/('+comp['ins']+' * 2);\n\n'

    text +='\tGth_W =-2.*M_PI*'+comp['l']+'*'+comp['kap']+'/log(Omegaold_CM);\n'
    ...
    text +='\tdH_II = '+comp['cp']+'*'+comp['tem']+'*((1. - (Omegaold_CM*Omegaold_CM ) + (2.*Omegaold_CM*Omegaold_CM*log(Omegaold_CM)))/(4.*Omegaold_CM*Omegaold_CM*(log(Omegaold_CM)*log(Omegaold_CM))));\n'
    text +='\tdH_III = 2. * '+comp['dh']+' * Omegaold_CM;\n'
    text +='\tdH_W = dH_I * (dH_II + dH_III);\n'
    text +='\tddH_I = 2. *'+comp['cp']+' * '+comp['tem']+' / (Omegaold_CM*log(Omegaold_CM));\n'
    text +='\tddH_II = -1.*'+comp['cp']+'*'+comp['tem']+'*(1.-(Omegaold_CM*Omegaold_CM )+(2.*Omegaold_CM*Omegaold_CM*log(Omegaold_CM)))/(2.*Omegaold_CM*Omegaold_CM*Omegaold_CM*log(Omegaold_CM)*log(Omegaold_CM)*log(Omegaold_CM));\n'
    text +='\tddH_III = -1.*'+comp['cp']+'*'+comp['tem']+'*(1.-(Omegaold_CM*Omegaold_CM )+(2.*Omegaold_CM*Omegaold_CM*log(Omegaold_CM)))/(2.*Omegaold_CM*Omegaold_CM*Omegaold_CM*log(Omegaold_CM)*log(Omegaold_CM));\n\n'

    text +='\tFwv =(1*(Omegaold_CM-Omegaxold_CM)/'+comp['hh']+')-(((0.99999 - Omegaold_CM)*(Omegaold_CM-0.000001))*((((Un_CM*Un_CM)/Rch_W)-(Gth_W*'+comp['tem']+'))/dH_W));\n'
    text +='\tJw = (1/'+comp['hh']+')+((0.99999 - Omegaold_CM)*(Omegaold_CM-0.000001))*(((((Un_CM*Un_CM)/Rch_W)-(Gth_W*'+comp['tem']+'))*((2.*'+comp['dh']+')+ddH_I+ddH_II+ddH_III))/(dH_I*((dH_II + dH_III)*(dH_II + dH_III))))-(((0.99999 - Omegaold_CM)*(Omegaold_CM-0.000001))*((dRch+dGth)/dH_W)) - ((0.99999 - Omegaold_CM)*(((Un_CM*Un_CM)/Rch_W)-(Gth_W*'+comp['tem']+'))/(dH_I*((dH_II + dH_III)))) + ((Omegaold_CM-0.000001)*((((Un_CM*Un_CM)/Rch_W)-(Gth_W*'+comp['tem']+'))/(dH_I*((dH_II + dH_III)))));\n'
    text +='\tJv = ((0.99999 - Omegaold_CM)*(Omegaold_CM-0.000001))*(-2.*Un_CM)/(dH_W*Rch_W);\n\n'
    ...
    text +='\tGeq_CM = (1./Rch_W) + ((Jv/Jw)*(dRch/(Rch_W*Rch_W)));\n'
    text +='\tIeq_CM = (dRch/(Rch_W*Rch_W))*IIeq_CM + (-1*((dRch/(Rch_W*Rch_W))*Omegaold_CM));\n'
